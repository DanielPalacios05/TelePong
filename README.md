...
- Con el fin de realizar un mejor seguimiento a las peticiones que el servidor procesa y response, se estableció un sistema de log, donde tanto en consola del servidor, como en este archivo se guardan tales peticiones junto con su fecha y hora.
...
- PLAYER CREATE <ins>_NICKNAME_</ins>: Permite el envío del mensaje junto con el _nickname_, al servidor para crear un nuevo jugador.
...
- PLAYER SEND_MOVE <ins>_SOCKET_</ins> <ins>_MESSAGE_</ins> (en el protocolo en Python): Permite que el cliente pueda enviar al servidor un mensaje (<ins>_MESSAGE_</ins>) que está relacionado con el movimiento que este realiza en el juego usando su _socket_ (<ins>_SOCKET_</ins>).
...
- PLAYER SEND_MOVE <ins>_MOVEMENT_</ins> <ins>_SOCKET_</ins> <ins>_PLAYER_ADD_</ins> (en el protocolo en C): Permite que el servidor envíe un movimiento que realizó un jugador de determinada partida a la dirección IP del oponente. Esto se logra usando el _socket_ del servidor (<ins>_SOCKET_</ins>), el movimiento que realizó el jugador (<ins>_MOVEMENT_</ins>), y la dirección IP junto con el puerto (<ins>_PLAYER_ADD_</ins>) del oponente respectivo.
- PLAYER SEND_WIN <ins>_MOVEMENT_</ins> <ins>_SOCKET_</ins> <ins>_PLAYER_ADD_</ins> (en el protocolo en C): Permite que el servidor envíe el mensaje que anuncia el ganador enviado por un cliente de determinada partida a la dirección IP del oponente. Esto se logra usando el _socket_ del servidor (<ins>_SOCKET_</ins>), el mensaje enviado por el jugador (<ins>_MOVEMENT_</ins>), y la dirección IP junto con el puerto (<ins>_PLAYER_ADD_</ins>) del oponente respectivo.
- PLAYER SEND_CONF <ins>_MOVEMENT_</ins> <ins>_SOCKET_</ins> <ins>_PLAYER_ADD_</ins> (en el protocolo en C): Permite que el servidor envíe un mensaje de confirmacion como respuesta al mensaje que anuncia el ganador enviado por un cliente de determinada partida a la dirección IP del oponente. Esto se logra usando el _socket_ del servidor (<ins>_SOCKET_</ins>), el mensaje enviado por el jugador (<ins>_MOVEMENT_</ins>), y la dirección IP junto con el puerto (<ins>_PLAYER_ADD_</ins>) del oponente respectivo.
...
- SERVER INIT_PLAYER <ins>_NICKNAME_</ins>: Permite la creación de un nuevo jugador, tomando como _nickname_ el mandado en el espacio correspondiente.
